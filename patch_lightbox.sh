...
.lightbox-close:hover { background: var(--purple); transform: scale(1.1); }
.lightbox-caption {
...
async function loadGallery() {
  ...
  galleryGrid.innerHTML = images.map(img => `
    <div class="gallery-thumb ${activeImage?.id === img.id ? 'active' : ''}"
      ...
      <img src="${img.thumb_url}" alt="" loading="lazy" ondblclick="event.stopPropagation();openLightbox(this.closest('[data-url]').dataset.url,this.closest('[data-url]').dataset.prompt)">
